    ...
    <div className="p-10">
      <div className="mb-2">
        <h1 className="text-page-title text-gray-900 font-semibold">
          {ProfileConfig.headerLabel}
        </h1>
      </div>
      <div className="shadow-level-2 w-full p-10">
        <div className="flex flex-col lg:flex-row items-center gap-10">
          {ProfileConfig.userInfo.map((item) => {
            return (
              <>
                <div
                  key={item.memberID}
                  className="rounded-full bg-primary-600 w-25 h-25"
                ></div>
                <div className="flex flex-col">
                  ...
                  <p className="text-p text-gray-500">
                    {`Member ID:${item.memberID}, ${item.dob}`}
                  </p>
                  <div className="bg-green-100 mt-3 rounded-sm p-1 text-center">
                    <p className="text-success">
                      {item.coverageActive ? CoverageActive : ""}
                    </p>
                  </div>
                </div>
              </>
              ...
              <div className=" pt-2">
                ...
                <div>
                  {plan.description.map((item, idx) => {
                    return (
                      <>
                        <p key={idx} className="text-body text-gray-700">
                          {item.planCategory}
                        </p>
